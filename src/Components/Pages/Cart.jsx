...
const Cart = () => {
  ...
  const handleRemoveFromCart = (id) => {
    const updatedCart = cartProducts.filter((e) => e.id !== id);
    ...
    return (
      <>
        ...
        <div className="h-[90vh] flex flex-col justify-center items-center text-4xl font-semibold">
          ...
          <NavLink to="/">
            <p className="p-3 bg-red-500 border-2 border-white rounded-2xl text-lg m-2 text-white hover:bg-white hover:text-red-500 hover:border-red-500 duration-300">
              Browse all products
            </p>
          </NavLink>
        </div>
        ...
        <div className="min-h-full w-full lg:w-2/3 flex flex-col items-center gap-5 shadow-lg">
          {cartProducts.map(({ id, image, title, price, quantity, thumbnail }) => {
            return (
              <div
                key={id}
                className="min-h-[180px] md:h-40 w-[90%] flex flex-col md:flex-row justify-evenly items-center rounded-2xl shadow-md mt-3 p-3 gap-3"
              >
                <img
                  src={image || thumbnail}
                  alt={title}
                ...
                <h2 className="text-sm md:text-base font-semibold text-neutral-800 text-center line-clamp-2">
                  {title}
                </h2>
                ...
                <button
                  ...
                  className="border-2 border-blue-950 px-4 py-2 rounded-3xl text-blue-950 font-semibold hover:bg-blue-950 hover:text-white hover:border-white cursor-pointer duration-300 w-auto"
                >
                  Remove
                </button>
              </div>
