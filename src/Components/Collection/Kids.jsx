...
const Kids = () => {
  ...
  useEffect(()=>{
    Promise.all([
     ...
     .then(([toys , dresses,mobile,sunglasses])=>{
        const allProducts =[
      ...toys.data.products,
...
>
  {wishlist.some((item) => item.id === product.id) ? "❤️" : "🤍"}
          ...
          {isPresent ? (
            ...
            <button onClick={()=>addToCart(product)} className="mt-3 bg-amber-500 text-white py-2 rounded-xl cursor-pointer text-xs sm:text-sm">Add to Cart</button>
          )
          }
        </div>
      )
